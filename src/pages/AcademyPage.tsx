  ...
  return (
    <main>
      <section className="bg-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-green/8 rounded-full blur-[100px] pointer-events-none" />
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-14 md:py-20">
          <div className="grid md:grid-cols-2 gap-6 lg:gap-10 items-center">
            <motion.div className="relative rounded-3xl overflow-hidden shadow-image border border-border" {...fadeUp}>
            ...
            <motion.div {...fadeUp}>
              <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green/10 text-[11px] font-bold text-green uppercase tracking-wider border border-green/20 mb-4">Academics</span>
              <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-text-primary leading-tight mb-4">A dedicated academic page for curriculum, learning outcomes, and grade-wise structure.</h1>
              <p className="text-sm text-text-secondary leading-relaxed mb-5">This page is ready for your upcoming academic content, including curriculum details, methodology, faculty focus, grade progression, labs, and performance philosophy.</p>
              <div className="flex flex-wrap gap-2">{["Concept clarity", "Activity-led learning", "Progress tracking"].map((c) => (<span key={c} className="px-3 py-1.5 rounded-full bg-accent/8 text-xs font-semibold text-text-secondary border border-border">{c}</span>))}</div>
            </motion.div>
          </div>
        </div>
      </section>
